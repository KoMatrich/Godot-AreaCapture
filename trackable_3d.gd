...
## Minimum distance the object must move (in meters) from its last reported position to trigger a new report.
...
func _process(delta: float) -> void:
	...
	if move_threshold > 0:
		if _last_reported_position != Vector3.INF:
...
func _register() -> void:
	if tag_name != "":
		print("[Trackable3D] _register: tag='%s' node='%s'" % [tag_name, name])
		PositionTracker.register_object(tag_name, self)

func _unregister() -> void:
	if tag_name != "" and is_inside_tree():
		print("[Trackable3D] _unregister: tag='%s' node='%s'" % [tag_name, name])
